...
int main( int argc, char** argv ){
  ...
  //ROS_INFO("The marker should be published");

  while (ros::ok()){
    if(!arrived_at_pickup){
      ROS_INFO("I am waiting for pickup!");
    }
   
    // --- Conditions while the robot is on its way toward the pickup location ---
    // Wait for the robot to reach the pickup location
    
    while(!arrived_at_pickup){
    	ros::spinOnce();
    }
    // Check if the robot arrived at the pickup location and completed the pickup; if so, remove the marker
    if(arrived_at_pickup && !completed_pickup){
      //remove the marker and begin moving to drop off location
      marker.action = visualization_msgs::Marker::DELETE;
      marker_pub.publish(marker);
      completed_pickup = true;
      ros::Duration(5.0).sleep();
      ROS_INFO("I have been picked up!");
    }
    
    // --- Conditions while the robot is on its way toward the dropoff location ---
    // Wait for the robot to reach the dropoff location
    if(!arrived_at_dropoff){
      ROS_INFO("I am waiting for dropoff!");
    }
    while(!arrived_at_dropoff){
      ros::spinOnce();
    }
    // Check if the robot has arrived at the dropoff location and completed the dropoff; if so, place the marker
    if(arrived_at_dropoff && !completed_dropoff){
      // Set the dropoff pose of the marker.  This is a full 6DOF pose relative to the frame/time specified in the header
      // Set the marker action.  Options are ADD, DELETE, and new in ROS Indigo: 3 (DELETEALL)
      marker.action = visualization_msgs::Marker::ADD; 
      ...
      marker_pub.publish(marker);
      completed_dropoff = true;
      ROS_INFO("I have been dropped off!");
